...
trait DMLPersistence {
	...
	public function addressJoin($isFull = true, $mArray = ['village' => 'village', 'ward' => 'ward']) {
		$cSelect = 'p.id as panchayat,p.pName,c.cityName,c.id as city,w.id as ward,s.id as s_district,s.subdistrictName,w.wardName,v.vName,COALESCE(v.vName,w.wardName)As villageWardName,COALESCE(c.cityName,p.pName)As panchayatCityName';
		$this->model->select($cSelect)->join('village as v', 'v.id = ' . $mArray["village"], 'left')
			->join('panchayat as p', 'p.id = v.panchayat_id', 'left')
			->join('ward as w', 'w.id = ' . $mArray["ward"], 'left')
			->join('city as c', 'c.id = w.city_id', 'left')
			->join('subdistrict as s', 's.id = p.sub_district_id OR s.id = c.sub_district_id', 'left');
		if ($isFull) {
			$this->model->select($cSelect . ',d.id as district,st.id as state,ct.id as country,ct.countryName,st.stateName,d.districtName')
				->join('district as d', 'd.id = s.district_id', 'left')
				->join('state as st', 'st.id = d.state_id', 'left')
				->join('country as ct', 'ct.id = st.country_id', 'left');
		}
	}
